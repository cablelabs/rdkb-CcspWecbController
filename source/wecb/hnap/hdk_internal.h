...
typedef int (*HDK_ReadFn)(void* pDeviceCtx, char* pBuf, int cbBuf);
...
/*
 * Parse data supplied in blocks.
 *   A typical call sequence would proceed:
 *
 *     HDK_ParseBegin();
 *     while (have data)
 *         HDK_ParseData(data);
 ...
 *     HDK_ParseEnd();
 */
extern HDK_ParseError HDK_ParseBegin(void* pDeviceCtx, HDK_ParseContext* pParseCtx, HDK_Struct* pStruct);
/* Pass 0 as data length to indicate no more data. */
extern HDK_ParseError HDK_ParseData(HDK_ParseContext* pParseCtx, char* pData, unsigned int cbData);
extern void HDK_ParseEnd(HDK_ParseContext* pParseCtx);

/* Parse data supplied by a read function. */
extern HDK_ParseError HDK_Parse(void* pDeviceCtx, HDK_ReadFn pfnRead,
                                HDK_Struct* pStruct, unsigned int cbContentLength);

/* HNAP XML serialization */
typedef void (*HDK_WriteFn)(void* pDeviceCtx, char* pBuf, int cbBuf);

extern int HDK_WriteBuf(void* pDeviceCtx, HDK_WriteFn pfnWrite, int fNoWrite, char* pBuf, int cbBuf);
extern int HDK_WriteBuf_Encode(void* pEncodeCtx, HDK_WriteFn pfnWrite, char* pBuf, unsigned int cbBuf);
extern int HDK_Write(void* pDeviceCtx, HDK_WriteFn pfnWrite, int fNoWrite, char* pszStr);
extern int HDK_Format(void* pDeviceCtx, HDK_WriteFn pfnWrite, int fNoWrite, char* pszStr, ...);
extern int HDK_Serialize(void* pDeviceCtx, HDK_WriteFn pfnWrite, int fNoWrite, HDK_Struct* pStruct, HDK_Struct* pInputStruct);
...
extern int HDK_Request_Validate(HDK_Struct* pStruct, HDK_Element topElement);
