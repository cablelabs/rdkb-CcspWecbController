...
int HDK_EncodeToBuffer(void* pEncodeCtx, HDK_WriteFn pfnWrite, char* pBuf, unsigned int cbBuf)
{
...
int HDK_EncodeString(HDK_EncodeFn pfnEncode, void* pEncodeCtx, HDK_WriteFn pfnWrite,
                     char* pBuf, unsigned int cbBuf)
{
    int cchLen = 0;
    ...
    while (pszValue != pszValueEnd)
    {
        ...
        if (pszEntity)
        {
            ...
            if (pfnEncode)
            {
                pfnEncode(pEncodeCtx, pfnWrite, pszValue, pszEntity - pszValue);
            }
            ...
            switch (*pszEntity)
            {
                case '"':
                    ...
                    if (pfnEncode)
                    {
                        pfnEncode(pEncodeCtx, pfnWrite, "&quot;", sizeof("&quot;") - 1);
                    }
                    break;
                case '&':
                    cchLen += sizeof("&amp;") - 1;
        ...
        else
        {
            ...
            if (pfnEncode)
            {
                pfnEncode(pEncodeCtx, pfnWrite, pszValue, pszValueEnd - pszValue);
            }
            break;
        }
    }
...
int HDK_EncodeBase64(HDK_EncodeFn pfnEncode, void* pEncodeCtx, HDK_WriteFn pfnWrite,
                     char* pBuf, unsigned int cbBuf, int* pState, int* pPrev)
{
    int cchLen = 0;
    ...
    for (p = pBuf; p != pBufEnd; p++)
    {
        if (*pState == 0)
        {
            if (pfnEncode)
            {
                pfnEncode(pEncodeCtx, pfnWrite, &s_szBase64[(*p & 0xFC) >> 2], 1);
            }
            cchLen += 1;
            *pState = 1;
            *pPrev = ((*p & 0x03) << 4);
        ...
        else if (*pState == 1)
        {
            if (pfnEncode)
            {
                pfnEncode(pEncodeCtx, pfnWrite, &s_szBase64[*pPrev + ((*p & 0xF0) >> 4)], 1);
            }
            cchLen += 1;
            *pState = 2;
            *pPrev = ((*p & 0x0F) << 2);
        ...
        else if (*pState == 2)
        {
            ...
            if (pfnEncode)
            {
                pfnEncode(pEncodeCtx, pfnWrite, c, sizeof(c));
            }
            cchLen += sizeof(c);
            *pState = 0;
        }
...
int HDK_EncodeBase64Done(HDK_EncodeFn pfnEncode, void* pEncodeCtx, HDK_WriteFn pfnWrite,
                         int state, int prev)
{
    int cchLen = 0;
    ...
    if (state)
    {
        ...
        if (pfnEncode)
        {
            pfnEncode(pEncodeCtx, pfnWrite, c, cchLen);
        }
    }
