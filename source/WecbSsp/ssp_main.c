...
PCCSP_COMPONENT_CFG             gpWecbStartCfg           = NULL;
...
int  cmd_dispatch(int  command)
{
    ...
    switch ( command )
    {
...

            {
                ...
                if ( g_Subsystem[0] != 0 )
                {
                    _ansc_sprintf(CName, "%s%s", g_Subsystem, gpWecbStartCfg->ComponentId);
                }
                else
                {
                    _ansc_sprintf(CName, "%s", gpWecbStartCfg->ComponentId);
                }
                ...
                ssp_WifiMbi_MessageBusEngage
                    ( 
                        CName,
                        CCSP_MSG_BUS_CFG,
                        gpWecbStartCfg->DbusPath
                    );
            }

#endif

                ssp_create_wifi(gpWecbStartCfg);
                ssp_engage_wifi(gpWecbStartCfg);
        ...
        case    'c':
                ssp_cancel_wifi(gpWecbStartCfg);
...
int main(int argc, char* argv[])
{
     ...
     */
    gpWecbStartCfg = (PCCSP_COMPONENT_CFG)AnscAllocateMemory(sizeof(CCSP_COMPONENT_CFG));
    ...
    if ( gpWecbStartCfg )
    {
        CcspComponentLoadCfg(CCSP_WIFI_START_CFG_FILE, gpWecbStartCfg);
    }
    else
    {
        exit(1);
