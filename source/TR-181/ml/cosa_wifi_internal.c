    ...
    )
{
     ...
     */
    pMyObject->Oid               = COSA_DATAMODEL_MOCA_WIFI_EXTENDER_OID;
...
#if 0
	pMyObject->disconnect_client = AnscAllocateMemory(1024);

	if(pMyObject->disconnect_client == NULL)
	{
		return ANSC_STATUS_FAILURE;
	}

	AnscZeroMemory(pMyObject->disconnect_client, 1024);
# endif
    ...
    )
{
	...
	CosaDmlWiFiRadioGetEntry((ANSC_HANDLE)pMyObject->hPoamWiFiDm, uIndex, &pWifiRadio->Radio);
    }
...
#if 0
	if (pMyObject->disconnect_client)
		AnscFreeMemory(pMyObject->disconnect_client);
#endif
